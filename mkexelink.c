...
static void xfree(void *p) { if (p) HeapFree(process_heap(), 0, p); }

static SIZE_T xwcslen(LPCWSTR s) { SIZE_T n = 0; while (s[n]) n++; return n; }
...
static void write_stderr(LPCWSTR s)
{
    ...
    DWORD written = 0;
    if (h != INVALID_HANDLE_VALUE && h != NULL) WriteFile(h, s, (DWORD)(xwcslen(s) * sizeof(WCHAR)), &written, NULL);
}
static void append_dec(WCHAR *buf, DWORD *pos, DWORD value)
{
    WCHAR tmp[10]; DWORD n = 0;
    do { tmp[n++] = (WCHAR)(L'0' + (value % 10)); value /= 10; } while (value);
    while (n) buf[(*pos)++] = tmp[--n];
}
static void die_last(LPCWSTR msg)
{
    WCHAR buf[256]; DWORD pos = 0;
    for (SIZE_T i = 0; msg[i] && pos < 180; i++) buf[pos++] = msg[i];
...
{
    if (a && a->argv) {
        for (int i = 0; i < a->argc; i++) xfree(a->argv[i]);
        xfree(a->argv); a->argv = NULL; a->argc = 0;
    }
    if (e && e->items) {
        for (DWORD i = 0; i < e->count; i++) { xfree(e->items[i].key); xfree(e->items[i].value); }
        xfree(e->items); e->items = NULL; e->count = 0;
    }
}
...
static LPWSTR argv_to_command_line(int argc, LPWSTR *argv)
{
    ...
    for (int i = 0; i < argc; i++) {
        ...
        for (SIZE_T j = 0; arg[j]; j++) {
            ...
            if (c == L'\\') { bs++; continue; }
            if (c == L'"') {
                for (SIZE_T k = 0; k < bs * 2 + 1; k++) if (!append_wchar(&b, L'\\')) goto fail;
                bs = 0; if (!append_wchar(&b, L'"')) goto fail; continue;
            }
            while (bs) { if (!append_wchar(&b, L'\\')) goto fail; bs--; }
            if (!append_wchar(&b, c)) goto fail;
        }
        if (quote) {
            while (bs) { if (!append_wchar(&b, L'\\') || !append_wchar(&b, L'\\')) goto fail; bs--; }
            if (!append_wchar(&b, L'"')) goto fail;
        } else while (bs) { if (!append_wchar(&b, L'\\')) goto fail; bs--; }
    }
    if (!append_wchar(&b, L'\0')) goto fail;
    return (LPWSTR)b.data;
fail:
    xfree(b.data); return NULL;
}
...
static int write_resource(LPCWSTR exe, BYTE *data, DWORD len)
{
    ...
    if (!h) { die_last(L"BeginUpdateResourceW"); return 0; }
    if (!UpdateResourceW(h, MAKEINTRESOURCEW(RT_RCDATA), MAKEINTRESOURCEW(CONFIG_RESOURCE_ID), MAKELANGID(LANG_NEUTRAL, SUBLANG_NEUTRAL), data, len)) {
        die_last(L"UpdateResourceW"); EndUpdateResourceW(h, TRUE); return 0;
    }
    if (!EndUpdateResourceW(h, FALSE)) { die_last(L"EndUpdateResourceW"); return 0; }
    return 1;
}
...
static int write_file(LPCWSTR path, const BYTE *data, DWORD len)
{
    ...
    if (h == INVALID_HANDLE_VALUE) { die_last(L"CreateFileW"); return 0; }
    DWORD written = 0; BOOL ok = WriteFile(h, data, len, &written, NULL) && written == len;
    CloseHandle(h); if (!ok) { die_last(L"WriteFile"); return 0; } return 1;
}
static int add_env(ENV_MAP *e, LPCWSTR spec)
{
    LPCWSTR eq = xwcschr(spec, L'=');
    if (!eq || eq == spec) { write_stderr(L"environment setting must be KEY=VALUE\r\n"); return 0; }
    SIZE_T key_chars = (SIZE_T)(eq - spec);
    LPWSTR key = (LPWSTR)xzalloc((key_chars + 1) * sizeof(WCHAR)); if (!key) return 0;
    memcpy(key, spec, key_chars * sizeof(WCHAR));
    ENV_PAIR *ni = (ENV_PAIR *)xrealloc(e->items, (e->count + 1) * sizeof(ENV_PAIR));
    if (!ni) { xfree(key); return 0; }
    e->items = ni; e->items[e->count].key = key; e->items[e->count].value = xwcsdup(eq + 1);
    if (!e->items[e->count].value) { xfree(key); e->items[e->count].key = NULL; return 0; }
    e->count++; return 1;
}
...
static int wmain_nocrt(int argc, wchar_t **wargv)
{
    ...
    LPCWSTR output = wargv[1]; ENV_MAP env = {0}; int command_index = -1;
    for (int i = 2; i < argc; i++) {
        if (xwcscmp(wargv[i], L"--") == 0) { command_index = i + 1; break; }
        if (xwcscmp(wargv[i], L"--env") == 0 && i + 1 < argc) {
            if (!add_env(&env, wargv[++i])) { free_config(NULL, &env); return 1; }
            continue;
        }
        usage(); free_config(NULL, &env); return 2;
    }
    if (command_index < 0 || command_index >= argc) { usage(); free_config(NULL, &env); return 2; }
    ARGV_LIST argv_prefix = {0}; argv_prefix.argc = argc - command_index;
    argv_prefix.argv = (LPWSTR *)xzalloc((SIZE_T)argv_prefix.argc * sizeof(LPWSTR));
    if (!argv_prefix.argv) { free_config(NULL, &env); return 1; }
    for (int i = 0; i < argv_prefix.argc; i++) {
        argv_prefix.argv[i] = xwcsdup(wargv[command_index + i]);
        if (!argv_prefix.argv[i]) { free_config(&argv_prefix, &env); return 1; }
    }
    BYTE_BUF blob = {0};
    ...
    xfree(blob.data); free_config(&argv_prefix, &env);
    if (!ok) { DeleteFileW(output); return 1; }
    return 0;
}
